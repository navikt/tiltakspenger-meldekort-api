    ...
    private fun vurderVarselForSak(sakForVurdering: SakForVarselvurdering) {
        ...
        Either.catch {
            vurderVarselForSak(
                ...
                hentVarsler = { varselRepo.hentForSakId(sak.id) },
                hentKjederSomManglerInnsending = { varselMeldekortRepo.hentKjederSomManglerInnsending(sak.id) },
                ...
                markerVarselVurdert = { vurdertTidspunkt, sistFlagget, sessionContext ->
                    sakVarselRepo.markerVarselVurdert(sak.id, vurdertTidspunkt, sistFlagget, sessionContext)
                },
                logInfo = { melding -> logger.info { melding } },
                logWarn = { melding -> logger.warn { melding } },
            )
        }.onLeft {
