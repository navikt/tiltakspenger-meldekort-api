...
import no.nav.tiltakspenger.meldekort.domene.varsler.utsettSendingTilHvisUtenforÅpningstid
...
class TmsVarselClientImpl(
    ...
    override fun sendVarsel(varselId: VarselId, fnr: Fnr): SendtVarselMetadata {
        ...
        val utsettSendingTil = finnUtsettSendingTil()
        val varselHendelse: String = opprettVarselOppgave(varselId, fnr, utsettSendingTil)
    ...
    private fun finnUtsettSendingTil(): ZonedDateTime? {
        return LocalDateTime.now(clock).utsettSendingTilHvisUtenforÅpningstid()?.atZone(zoneIdOslo)
    }
