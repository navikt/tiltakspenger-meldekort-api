...
class SakVarselPostgresRepoTest {
    ...
    fun `hentSakerSomSkalVurdereVarsel returnerer bare flaggede saker sortert på id og begrenset av limit`() {
        withMigratedDb { helper ->
            ...
            resultat.map { it.sak.id } shouldContainExactly forventet.map { it.id }
            resultat.map { it.sak.meldeperioder.single().id } shouldContainExactly forventet.map { it.meldeperioder.single().id }
        }
    }
    ...
    fun `flaggForVarselvurdering setter flagget tilbake til true`() {
        withMigratedDb { helper ->
            ...
            helper.sakVarselPostgresRepo.hentSakerSomSkalVurdereVarsel().map { it.sak.id } shouldContainExactly listOf(sak.id)
            hentSakVarselstatus(helper, sak.id) shouldBe SakVarselstatus(
    ...
    fun `markerVarselVurdert kaster OptimistiskLåsFeil når sist_flagget_tidspunkt er endret siden lesing`() {
        withMigratedDb { helper ->
            ...
            helper.sakVarselPostgresRepo.hentSakerSomSkalVurdereVarsel()
                .map { it.sak.id } shouldContainExactly listOf(sak.id)
        }
    }
