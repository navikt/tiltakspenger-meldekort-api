...
/**
 * Test-double for in-memory-kontekst (uten ekte Postgres): statusoppdatering er no-op, og
 * lesespørringene returnerer [MicrofrontendFeil.DatabaseFeil] slik at feilhåndteringen i lagene over
 * kan testes uten en ekte databasefeil. De faktiske spørringene dekkes av tester mot ekte Postgres.
 */
class MicrofrontendRepoFake : MicrofrontendRepo {
    ...
    override fun hentSakerHvorMicrofrontendSkalAktiveres(limit: Int, sessionContext: SessionContext?): Either<MicrofrontendFeil, List<MicrofrontendSak>> {
        // Aggregerte spørringer testes mot ekte Postgres. In-memory returnerer en feil i stedet for å kaste.
    ...
    override fun hentMeldekortInfo(fnr: Fnr, sessionContext: SessionContext?): Either<MicrofrontendFeil, MicrofrontendMeldekortInfo> {
        // Spørringen testes mot ekte Postgres. In-memory returnerer en feil slik at route-laget
        // sin feilhåndtering (500) kan dekkes uten en ekte databasefeil.
        return MicrofrontendFeil.DatabaseFeil(NotImplementedError("Microfrontend-spørringene testes mot ekte Postgres, ikke in-memory.")).left()
    }
}
